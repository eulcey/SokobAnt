...
(defun init-controls ()
  ...
  (gamekit:bind-button :z :released (lambda () (revert-one-state-stop))))
...
(defun handle-player-move (level items targets)
  (if (not *paused*)
      (let ((new-x (if (caddr *pressed-directions*)
		       (1+ (gamekit:x (get-player-position *state-stack*)))
		       (if (cadddr *pressed-directions*)
			   (1- (gamekit:x (get-player-position *state-stack*)))
			   (gamekit:x (get-player-position *state-stack*)))))
	    (new-y (if (car *pressed-directions*)
		       (1+ (gamekit:y (get-player-position *state-stack*)))
		       (if (cadr *pressed-directions*)
			   (1- (gamekit:y (get-player-position *state-stack*)))
			   (gamekit:y (get-player-position *state-stack*))))))
	  ...
	  (let ((new-position (gamekit:vec2 new-x new-y)))
		...
		(let ((obj (get-object items new-position)))
		  (if (not obj) ; tile is completely free, player can move
		      ...
		      (let ((push-dir (gamekit:subt new-position (get-player-position *state-stack*))))
		      (when (obj-can-be-pushed obj push-dir
					       level items)
			(player-move-with-obj new-x new-y obj push-dir)))))))
...
(defun player-move (x y)
  (let ((new-player-pos (gamekit:vec2 x y))
	(old-items (get-items *state-stack*)))
    (push-new-state new-player-pos old-items)))

(defun push-object (object push-vector)
  (setf (caddr object)
	(calc-new-obj-pos object push-vector)))

(defun calc-new-items (id new-obj-pos old-items)
  (mapcar (lambda (item)
	    (if (eql id (car item))
		(list (get-object-id item)
		      (get-object-type item)
		      new-obj-pos)
		(list (get-object-id item)
		      (get-object-type item)
		      (get-object-position item))))
	  old-items))
  
(defun player-move-with-obj (x y obj push-dir)
  (let* ((new-player-pos (gamekit:vec2 x y))
	 (old-items (get-items *state-stack*))
	 (new-obj-pos (calc-new-obj-pos obj push-dir))
	 (new-items (calc-new-items (get-object-id obj) new-obj-pos old-items)))
    (push-new-state new-player-pos new-items)))
